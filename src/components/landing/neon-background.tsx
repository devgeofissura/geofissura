  ...
  useEffect(() => {
    ...
    try {
      ...
      for (let i = 0; i < totalVerts; i++) {
        ...
        U[gp] = inst.x + S[gp]; U[gp + 1] = inst.y + S[gp + 1]; U[gp + 2] = inst.z + S[gp + 2]
      }
      ...
      const update = (now: number) => {
        ...
        ctx.animId = requestAnimationFrame(update)
      }
    ...
    return () => {
      try { cancelAnimationFrame(ctx.animId) } catch {}
      try { window.removeEventListener("pointermove", ctx.onPointer) } catch {}
      try { window.removeEventListener("resize", ctx.onResize) } catch {}
      try { ctx.pg?.dispose(); ctx.lg?.dispose(); ctx.lm?.dispose(); ctx.pm?.dispose() } catch {}
      try {
        const r = ctx.renderer
        if (r) { r.dispose(); if (el.contains(r.domElement)) el.removeChild(r.domElement) }
      } catch {}
    }
  }, [])
