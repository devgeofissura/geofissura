...
import { Loader2, Plus, Trash2, Shield, ShieldCheck, Building2, Pencil, Check, ArrowLeft, Users, KeyRound, Cpu, HardDrive } from "lucide-react"
...
export default function AdminPage() {
  const [tab, setTab] = useState<"usuarios" | "clientes" | "tipos" | "equipamentos">("clientes")
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-[var(--border)] pb-2">
        ...
        <TabButton active={tab === "equipamentos"} onClick={() => setTab("equipamentos")}>Tipos de Equipamento</TabButton>
      </div>
      {tab === "clientes" && <ClientesTab />}
      {tab === "usuarios" && <UsuariosGlobalTab />}
      {tab === "tipos" && <TiposSensorTab />}
      {tab === "equipamentos" && <TiposEquipamentoTab />}
    </div>
  )
}
...
function TiposEquipamentoTab() {
  const [tipos, setTipos] = useState<{ id: number; nome: string; descricao: string | null }[]>([])
  const [loading, setLoading] = useState(true)
  const [showForm, setShowForm] = useState(false)
  const [saving, setSaving] = useState(false)
  const [editingId, setEditingId] = useState<number | null>(null)
  ...
  async function handleEditSave(id: number) {
    try {
      const res = await fetch(`/api/tipos-equipamento/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nome: editNome, descricao: editDescricao }),
      })
  ...
  async function handleDelete(id: number, nome: string) {
    if (!confirm(`Excluir tipo "${nome}"?`)) return
    try {
      await fetch(`/api/tipos-equipamento/${id}`, { method: "DELETE" })
      toast.success("Tipo excluído")
      load()
    } catch {
      toast.error("Erro ao excluir")
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-[var(--text-secondary)]">Tipos de equipamento cadastrados</p>
        <Button onClick={() => setShowForm(!showForm)} size="sm"><Plus className="mr-1 h-4 w-4" />Novo Tipo</Button>
      </div>

      {showForm && (
        <form onSubmit={handleCreate} className="max-w-lg rounded-xl border border-[var(--border)] bg-[var(--bg-primary)] p-6 shadow-sm space-y-4">
          <h3 className="font-semibold">Cadastrar Tipo de Equipamento</h3>
          <div className="space-y-2">
            <Label>Nome</Label>
            <Input name="nome" required placeholder="Ex: Módulo WiFi" />
          </div>
          <div className="space-y-2">
            <Label>Descrição</Label>
            <Input name="descricao" placeholder="Descrição opcional" />
